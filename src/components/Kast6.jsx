...
export default function Kast6(props) {
  const { nodes, materials } = useGLTF("/models/kast6.gltf");
  ...
  useFrame((_state, delta) => {
    ...
    const targetScale = new THREE.Vector3(
      1,
      1,
      (60 * closetWidthScale + 3.6 * (closetWidthScale - 1)) / 60
    );
    ...
    const rightSideWidth =
      right.current.geometry.boundingBox.max.z -
      right.current.geometry.boundingBox.min.z;
    const targetRightSide = new THREE.Vector3(
      0,
      0,
      -0.58 - (rightSideWidth * targetScale.z - rightSideWidth)
    );

    // const targetRightSide = new THREE.Vector3(
    //   0,
    //   0,
    //   -0.6 * closetWidthScale + 0.018
    // );
...
useGLTF.preload("/models/kast6.gltf");
